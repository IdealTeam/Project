...
//MODIFICATION D'UN POST DANS PROFIL
   ...
   if (isset($_POST['modif_offre']))
   {
     ...
     if (isset($_GET['modifoffre']) && isset($_GET['ido']))
     {
       ...
       $db = convert_date_US($_POST['date_debut_modif_offre']);
       $lib = $_POST['libelle_modif_offre'];

       $idu = $_SESSION['UTILISATEUR'];
       echo $ville;
       $villeO = new commune ('','','','','');
       $sqlvilleO = "SELECT id_commune FROM vue_commune WHERE nom_commune = '$ville';";
       $reqvilleO = $villeO->sql_commune($sqlvilleO,$conn);
       $data_villeO = $reqvilleO->fetch();
       $commune_selectionneO = $data_villeO['id_commune'];
       /*if ($Stage alors )
       {
       ...
       SET titre_offre = '$titre',
        ...
        id_commune = '$commune_selectionneO'
       WHERE etat_offre = 1
       AND id_user = '$idu'
